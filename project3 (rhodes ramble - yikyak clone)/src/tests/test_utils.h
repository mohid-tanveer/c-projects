...
// tests.
int run_tests(int fail_fast, int clean_after);
...
#define EXPECT_STR_OP(val0, val1, OP)                                          \
  do {                                                                         \
    int r = strcmp(val0, val1);                                                \
    if (r OP 0) {                                                              \
    } else {                                                                   \
      fprintf(stderr, "[%s:%d] Assertion %s failed (result %d; %s %s %s)\n",   \
              __FILE__, __LINE__, "" #val0 " " #OP " " #val1 "", r, val0,      \
              "" #OP "", val1);                                                \
      return 1;                                                                \
    }                                                                          \
  } while (0);
...
#define EXPECT_STR_GTE(val0, val1) EXPECT_STR_OP(val0, val1, >=)
